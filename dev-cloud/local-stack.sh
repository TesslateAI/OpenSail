...
DATABASE="voie_dev"
PG_USER="voiedev"
ENDPOINT_HOST="voiedevlocal.blob.localhost"
...
# PUT container succeeds against its host-style endpoint. Prints the Azurite
# pid on success.
start_azurite() {
	...
	for _ in $(seq 1 30); do
		status=$(create_container_status "$ENDPOINT_HOST:$blob_port" "$ACCOUNT" "$CONTAINER" "$blob_key" || true)
		case "$status" in
			201 | 409)
				ready=1
				break
...
cmd_up() {
	...
	# Floci-AZ addresses accounts as path segments on the unified port;
	# Azurite keeps its host-style vanity name.
	local blob_endpoint
	blob_endpoint="http://$ENDPOINT_HOST:$blob_port"
	if [[ "$EMULATOR" == floci-az ]]; then
