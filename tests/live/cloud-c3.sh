...
#
# This script owns its full process lifecycle: it builds voie-cloud, spawns
# it against REAL PostgreSQL/Azure-Blob/mTLS-Fabric/model boundaries with the
# in-repo ephemeral OIDC issuer for the Web-session boot, restarts the
# control mid-proof, and tears everything down. Missing boundaries fail
# closed; nothing degrades to a mock.
set -euo pipefail
