...
#
# After this script passes, closing public SSH (management CIDRs to []) is
# applied by the deployment path; it is not asserted here.
set -euo pipefail
...
echo "live-c8 pass: isolation, guest credential isolation, unknown/no-replay (${ELAPSED}s), recovery, restore, cleanup all proven"
